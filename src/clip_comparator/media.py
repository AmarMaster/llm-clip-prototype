...
def download_video_to_sources(url: str, sources_dir: Optional[str] = None) -> str:
    """
    Download full episode video to sources/episode.mp4.
    Handles direct .mp4 downloads and .m3u8 via ffmpeg (requires ffmpeg installed).
    """
    if not url.strip():
        raise ValueError("Missing video URL.")
    sources_dir = sources_dir or os.path.join(os.getcwd(), "sources")
    _ensure_dir(sources_dir)
    out_path = os.path.join(sources_dir, f"{_sha1(url)}.mp4")
    ...
    if url.lower().endswith(".m3u8"):
        ...
        if res.returncode != 0 or not os.path.exists(out_path):
            raise RuntimeError(f"ffmpeg download failed: {res.stderr.decode('utf-8', errors='ignore')[:400]}")
        return out_path

    # Direct download (best effort). For non-mp4, still save as mp4 to simplify downstream.
    if requests is None:
        raise ImportError("`requests` not installed.")
...
def _ffmpeg_reencode_cut(src: str, start_s: float, end_s: float, dest: str):
    ...
    IMPORTANT: Use -t (duration) instead of -to (absolute time) to avoid huge/random clips.
    """
    ...
    res = subprocess.run(cmd, capture_output=True)
    if res.returncode != 0:
        raise RuntimeError(f"ffmpeg cut failed: {res.stderr.decode('utf-8', errors='ignore')[:400]}")
...
def render_clips_with_moviepy(
    ...
    """
    For each clip with start_ms/end_ms, cut the media using moviepy's ffmpeg_extract_subclip,
    or ffmpeg re-encode if force_reencode=True.
    Returns list of (clip, output_path_or_None).
    """
    _ensure_moviepy()
    ...
    for c in clips:
        ...
        if force_reencode:
            ...
            # moviepy uses ffmpeg under the hood; it expects seconds (floats ok)
...
def show_rendered_clips_in_order(
    ...
    """
    Streamlit display: show in order with playable widgets in compact expanders.
    Uses st.video/st.audio depending on extension (best-effort).
    """
    st.subheader(title)
    ...
    for idx, (clip, path) in enumerate(rendered, 1):
        ...
        if path is None:
            st.warning("No timestamps available for this clip, so it cannot be cut.")
